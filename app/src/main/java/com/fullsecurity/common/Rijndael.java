...
public final class Rijndael {
    ...
    public static final int DIR_ENCRYPT = 1;                         // Flag to setup the encryption key schedule.
    public static final int DIR_DECRYPT = 2;                         // Flag to setup the decryption key schedule.
    public static final int DIR_BOTH =    (DIR_ENCRYPT|DIR_DECRYPT); // Flag to setup both key schedules (encryption/decryption).
    public static final int BLOCK_BITS  = 128;                       // AES block size bufferedReaderStream bits (the Rijndael algorithm allows for other sizes)
    public static final int BLOCK_SIZE  = (BLOCK_BITS >>> 3);        // AES block size bufferedReaderStream bytes (the Rijndael algorithm allows for other sizes)
    ...
    // Round constants
    private static final int[] rcon = new int[10]; // Rijndael never uses more than 10 rcon values
    
    private int Nr = 0; // Number of rounds (depends on key size).
    private int Nk = 0;
    private int Nw = 0;
    
    private int rek[] = null; // Encryption key schedule
    
    private int rdk[] = null; // Decryption key schedule
    ...
    // (only worthwhile if Rijndael is used bufferedReaderStream a hashing function mode)
    /*
    private void expandKey(byte[] cipherKey) {
        int keyOffset = 0;
            int i = 0;
    ...
    */
    ...
    // Set up the AES key schedule for encryption, decryption, or both
    // cipherKey:  the cipher key (128, 192, or 256 bits)
    // keyBits:    size of the cipher key bufferedReaderStream bits
    ...
    public void makeKey(byte[] cipherKey, int keyBits, int direction) throws RuntimeException {
        ...
        if ((direction & DIR_BOTH) != 0) {
            ...
            /*
            for (int r = 0; r <= Nr; r++) {
                System.printWriterStream.print("RK" + r + "=");
                for (int i = 0; i < 4; i++) {
                        int w = rek[4*r + i];
                        System.printWriterStream.print(" " + Integer.toHexString(w));
                }
                System.printWriterStream.println();
            }
             */
    ...
    // Set up the AES key schedule (any cipher direction)
    // cipherKey:  the cipher key (128, 192, or 256 bits)
    // keyBits:    size of the cipher key bufferedReaderStream bits
    ...
    public byte[] encryptArray(byte[] message) {
        int length = message.length; // Length of message to process
        int numOfBlocks = length / BLOCK_SIZE; // Number of whole blocks
        int lengthOfLastPart = length % BLOCK_SIZE; // Length of the last part (lolp)
        // 0 <= lolp <= BLOCK_SIZE-1
        ...
        byte[][] cArray = new byte[numOfBlocks][BLOCK_SIZE];
        for (int i = 0; i < numOfBlocks; i++) {
            System.arraycopy(message, i*BLOCK_SIZE, block, 0, BLOCK_SIZE);
            encryptBlock(block, cArray[i]);
        }
        // Now do the last part (or last block if message was multiple of BLOCK_SIZE)
        byte last[] = new byte[lengthOfLastPart];
        System.arraycopy(message, numOfBlocks*BLOCK_SIZE, last, 0, lengthOfLastPart);
        // 1 <= last.length <= BLOCK_SIZE
        int numOfPads = BLOCK_SIZE - last.length; // Number of padding bytes required
        // 0 <= numPads <= BLOCK_SIZE-1
        if (numOfPads == 0) numOfPads = BLOCK_SIZE;
        // 1 <= numPads <= BLOCK_SIZE
        byte[] result = new byte[numOfBlocks*BLOCK_SIZE+(numOfPads==BLOCK_SIZE?2:1)*BLOCK_SIZE];
        for (int i = 0; i < numOfBlocks; i++) System.arraycopy(cArray[i], 0, result, i*BLOCK_SIZE, BLOCK_SIZE);
        // Set the padding bytes
        byte pads[] = new byte[numOfPads];
        for (int i = 0; i < numOfPads; i++) pads[i] = (byte) numOfPads;
        if (numOfPads == BLOCK_SIZE) {
            System.arraycopy(encryptBlock(last, new byte[BLOCK_SIZE]), 0, result, numOfBlocks*BLOCK_SIZE, BLOCK_SIZE);
            System.arraycopy(encryptBlock(pads, new byte[BLOCK_SIZE]), 0, result, (numOfBlocks+1)*BLOCK_SIZE, BLOCK_SIZE);
        } else {
            last = Utilities.concatenateByteArrays(last, pads);
            System.arraycopy(encryptBlock(last, new byte[BLOCK_SIZE]), 0, result, numOfBlocks*BLOCK_SIZE, BLOCK_SIZE);
        }
        return result;
    }
    
    public byte[] decryptArray(byte[] message) {
        int length = message.length; // Length of message to process
        int numOfBlocks = length / BLOCK_SIZE; // Number of whole blocks
        int lengthOfLastPart = length - (numOfBlocks * BLOCK_SIZE); // Length of the last part
        // If message was mutliple of BLOCK_SIZE
        if(lengthOfLastPart == 0) {
            lengthOfLastPart = BLOCK_SIZE;
            numOfBlocks = numOfBlocks - 1;
        ...
        byte[] result = new byte[numOfBlocks*BLOCK_SIZE+lastBlock.length];
        for (int i = 0; i < numOfBlocks; i++) System.arraycopy(cArray[i], 0, result, i*BLOCK_SIZE, BLOCK_SIZE);
        System.arraycopy(lastBlock, 0, result, numOfBlocks*BLOCK_SIZE, lastBlock.length);
        return result;
    }
