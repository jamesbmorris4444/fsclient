...
public class SDVRequestProcessorComplexLoop1 extends MSCommunicationsWrapper {

    private static final String nameUsedForOutputToLogger = "complexLoop1Microservice: ";
    private static final String calledMicroServiceName2 = "complexloop2";

    private byte[] incoming;
    private String dbResult;
    private int clientId;
    private SDVRequestProcessorComplexLoop2 ms;
    private int msIndex;

    public SDVRequestProcessorComplexLoop1(int port) { super(port, nameUsedForOutputToLogger); }
    ...
    public void setOutputPayloads(Payload request) {
        ...
        request.setMSName(calledMicroServiceName2);
        msIndex = mapFromMSNameToMSIndex(msMap, calledMicroServiceName2);
        if (msIndex < 0) {
            debugLogOutput(incoming, "exit with ERR msINdex < 0");
            processReturnValue("ERR:" + (-UNABLE_TO_FIND_MICROSERVICE) + "|" + calledMicroServiceName2);
        } else {
            ms = (SDVRequestProcessorComplexLoop2) startMicroservice(msIndex);
            if (ms == null) {
